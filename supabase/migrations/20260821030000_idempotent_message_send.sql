-- Stable client-generated message IDs make retries idempotent without ever
-- storing plaintext outside the sender's browser memory.
...
-- SECURITY INVOKER keeps the existing message INSERT RLS authoritative.
...
begin
  ...
  select * into existing from public.messages where messages.id = message_id;
  if found then
